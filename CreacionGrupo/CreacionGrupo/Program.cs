...
namespace Creador_de_grupos
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int n = 0;
            int y = Convert.ToInt32(Math.Ceiling((Convert.ToDouble(Temas.Length) / Grupos.Length)));
            string[,] tabla = new string[Convert.ToInt32(1 + y + Math.Ceiling((Convert.ToDouble(Estudiantes.Length) / Grupos.Length))), Grupos.Length];
            ...
            n = 0;
            for (int i = y + 1; i < tabla.GetLength(0); i++)
            {
